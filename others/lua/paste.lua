...
local function write_struct_data(text, expire_time, ip)
    local data = {}
    data["text"] = text
    data["ip"] = ip
    data["expire"] = os.time() + expire_time
    data["time"] = os.time()
...
if ngx.var.request_method == "POST" then
    ...
    expire_time = math.max(0, tonumber(body["expire_time"]))
    local text = body["text"]
    if string.len(text) > 10 * 1024 * 1024 then
        rsp["success"] = false
        rsp["message"] = string.format("max length 10MB, but received %dB", string.len(text))
    else
        write_struct_data(text, expire_time, ngx.var.remote_addr)
        rsp["success"] = true
    end
